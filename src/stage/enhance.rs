use opencv::core::{BORDER_CONSTANT, Mat, Size};
use opencv::imgproc;
use opencv::prelude::*;
...
impl Stage for Enhance {
    ...
    fn process(&self, state: &mut FrameState) -> Result<(), StageError> {
        ...
        imgproc::cvt_color(warped, &mut gray, imgproc::COLOR_BGR2GRAY, 0).map_err(|e| {
            StageError {
                stage: self.name(),
                message: e.to_string(),
            }
        ...
        let mut clahe =
            imgproc::create_clahe(self.clip_limit, Size::new(self.grid_size, self.grid_size))
        ...
        clahe.apply(&gray, &mut enhanced).map_err(|e| StageError {
            stage: self.name(),
            message: e.to_string(),
        })?;
